...
def set_dataset_paths(args):
    ...
    if not args.train_path:
        args.train_path = 'data/%s/train' % (args.experiment)

    if not args.val_path:
        if (args.experiment in ['imagenet', 'face_verification', 'emotion', 'gender'] or
            args.experiment[:3] == 'age'):
            args.val_path = 'data/%s/val' % (args.experiment)
        else:
            args.val_path = 'data/%s/test' % (args.experiment)
